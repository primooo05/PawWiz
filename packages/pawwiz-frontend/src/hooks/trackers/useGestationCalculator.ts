...
import { gestationSchema } from '../../schemas/features';
...
export function useGestationCalculator() {
  const form = useFormValidation(gestationSchema, { 
  ...
  const handleCalculateGestation = (e: React.FormEvent) => {
    ...
    const mating = new Date(form.values.matingDate);
  ...
  return { form, gestationResult, handleCalculateGestation };
}